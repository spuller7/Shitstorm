...
Shader "Custom/Mirza Beig/Particles/Distortion/Alpha Blended Texture" {
Properties {
	...
	_distortion ("Distortion", range(0.0, 128.0)) = 10.0
}
	...
	SubShader {
			...
			{				
				...
				// Soft particles.
					...
					colour.a *= saturate(_InvFade * (sceneZ - partZ));
				...
				//distortionColour.rgb *= 4.0f; // Distortion not very visible otherwise.

				distortionColour.rgb = (distortionColour.rgb + colour.rgb) / 2.0f;
				distortionColour.a = colour.a * 2.0f;

				//distortionColour = lerp(colour, distortionColour, 0.5f);
				//distortionColour.a = colour.a;
